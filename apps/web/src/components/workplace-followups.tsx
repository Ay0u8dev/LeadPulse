...
export function WorkplaceFollowups({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    try {
      await workplace.propose({ leadId, title, details });
      setTitle("");
      setDetails("");
    } catch (error) {
      setError(
  ...
  return (
    <section className="ck-followups" aria-labelledby="followup-title">
      <header className="ck-followups-header">
        <div>
          <h2 id="followup-title">Sales follow-ups</h2>
          <p className="ck-local-note">
            Proposals are saved only after approval. Refresh reads the provider again.
          </p>
        </div>
      ...
      {status?.status === "unconfigured" ? (
        <div className="ck-setup-note">
          ...
          <p>{status.message}</p>
          <p>
            You can still inspect leads. No browser-only task will
      ...
      {tasks.length ? (
        ...
        <p className="ck-empty">No saved follow-ups for this lead.</p>
      ) : null}
      ...
      <form onSubmit={submit} className="ck-task-form ck-task-form--stacked">
        ...
        <input
          ...
          maxLength={200}
          placeholder="Write a sales-task title…"
          required
        />
        ...
        <textarea
          ...
          maxLength={4000}
          placeholder="What should the sales rep do, and why?"
          required
          rows={3}
        />
        <button
