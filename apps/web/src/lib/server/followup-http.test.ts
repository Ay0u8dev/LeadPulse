...
const proposal = {
  operation: "propose",
  leadId: "lead-yasmine",
  title: "Call Yasmine about a Bouskoura visit",
  details: "Use the selected lead context.",
};
...
test("HTTP proposal/approval/read flow rejects edited fields and never writes on reads", async (t) => {
  ...
  const refreshed = await handler(
    new Request(`${origin}/api/followups?leadId=lead-yasmine`, {
      headers: { cookie },
    }),
  );
...
test("cleanup failures do not mask successful provider responses", async (t) => {
      ...
      return [
        {
          ...
          title: "Call Yasmine about a Bouskoura visit",
          description: "leadpulse:lead-yasmine",
          url: null,
        },
      ];
    },
  ...
  assert.equal(response.status, 200);
  assert.equal(
    (await response.json()).tasks[0].title,
    "Call Yasmine about a Bouskoura visit",
  );
