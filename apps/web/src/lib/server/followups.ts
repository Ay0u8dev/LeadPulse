...
import { z } from "zod";
import { getLead, setApprovedTask } from "./lead-store";
...
const marker = (leadId: string) => `leadpulse:${leadId}`;
...
export class FollowupService {
  ...
  async propose(session: string, input: unknown): Promise<Proposal> {
    ...
    if (!lead)
      throw new FollowupError(
        "The selected lead no longer exists. Refresh the inbox and try again.",
      );
    ...
    const proposal = {
      ...
      leadId: lead.id,
      title: draft.title,
      description: `${draft.details}\n\nLead: ${lead.displayName || lead.phoneE164}\nStatus: ${lead.status} · Intent ${lead.intentScore}/100\n${marker(lead.id)}\nfollowup:${actionKey}`,
  ...
  async approve(session: string, id: string): Promise<WorkplaceTask> {
    ...
    const recordApprovedTask = (task: WorkplaceTask) => {
      setApprovedTask(proposal.leadId, task.id);
      return task;
    };
    const existing = await reconcile();
    if (existing) return recordApprovedTask(await readBack(existing.id));
    let sent = false;
    let conflict = false;
    ...
    try {
      ...
      if (conflict) {
        ...
        if (recovered) return recordApprovedTask(await readBack(recovered.id));
      }
      if (sent || conflict) throw uncertain();
      throw error; // A guaranteed pre-send failure remains retryable.
    }
    try {
      return recordApprovedTask(await readBack(created.id));
    } catch (error) {
