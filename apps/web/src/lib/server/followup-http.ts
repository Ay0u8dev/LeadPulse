...
const setup =
  "Set AMBIGUOUS_API_KEY in root .env and restart the web app. Lead qualification still works without it; sales-task approval requires a real Ambiguous workspace.";
      ...
      if (request.method === "GET") {
        ...
        return await withCleanup(
          reply({
            ...
            tasks: await service.list(url.searchParams.get("leadId") ?? "lead-yasmine"),
          }),
        );
      }
      ...
      if (error instanceof z.ZodError || error instanceof SyntaxError)
        return await withCleanup(
          reply(
            {
              error:
                "Invalid request or provider data. Check the lead, title, details, and record ID.",
            },
            400,
          ),
        );
