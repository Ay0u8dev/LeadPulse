...
const input = {
  leadId: "lead-yasmine",
  title: "Call Yasmine about a Bouskoura T3 visit",
  details: "Confirm her preferred visit time and financing readiness.",
};
...
test("concurrent approval and restart cannot duplicate a saved task; refresh reads the provider", async (t) => {
  ...
  assert.equal((await restarted.list("lead-yasmine"))[0].id, provider.tasks[0].id);
...
test("invalid proposal inputs fail before provider writes", async (t) => {
  ...
  await assert.rejects(
    service.propose(session, { ...input, leadId: "unknown" }),
  );
