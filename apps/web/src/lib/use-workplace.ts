...
export function useWorkplace(leadId: string) {
  ...
  const selectedLead = useRef(leadId);
  selectedLead.current = leadId;
  const refresh = useCallback(async () => {
    const leadId = selectedLead.current;
    ...
    try {
      ...
      if (
        ...
        leadId === selectedLead.current
      )
        setSnapshot({ leadId, status });
      return status;
    } catch (error) {
      if (
        request === sequence.current &&
  ...
  const propose = useCallback(
    async (draft: { leadId: string; title: string; details: string }) => {
      try {
  ...
  const status =
    snapshot?.leadId === leadId ? snapshot.status : undefined;
  return {
    status,
    proposal,
    error,
